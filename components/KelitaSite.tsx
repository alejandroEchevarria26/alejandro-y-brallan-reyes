    ...
    <header className="site-header">
      <a className="brand" href={isAdmin ? "#panel" : "#inicio"} onClick={closeMenu} aria-label="Tortas Kelita, inicio">
        ...
        <span>
          ...
          <small>La tradicion de la abuela</small>
        </span>
      </a>
      ...
      <nav className={open ? "site-nav is-open" : "site-nav"} aria-label="Navegacion principal">
        {isAdmin ? (
          <>
          ...
          <>
            ...
            <a href="#resenas" onClick={closeMenu}>Resenas</a>
...
// All catalog fields are controlled so validation and reset behavior stay predictable.
function CatalogManager({ productCount, onAddProduct }: CatalogManagerProps) {
  ...
  const handleImageChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    // La imagen se rechaza antes de leerla cuando supera el limite solicitado.
    if (file.size > MAX_IMAGE_SIZE) {
      setImageError("La imagen supera el tamano permitido de 2MB.");
      setImagePreview("");
      setImageName("");
      event.target.value = "";
      return;
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!form.name.trim() || !form.description.trim()) {
      setFormMessage("El nombre y la descripcion son obligatorios.");
      return;
    }
    ...
    if (!Number.isInteger(stock) || stock < 0) {
      setFormMessage("El stock debe ser un numero entero mayor o igual a 0.");
      return;
    }

    if (!imagePreview) {
      setFormMessage("Selecciona una imagen valida de hasta 2MB.");
      return;
    }
  ...
  return (
    <section id="gestion" className="section manager-section">
      <div className="manager-heading">
        <div>
          <span className="eyebrow">Gestion del catalogo</span>
          <h2>Agrega un nuevo producto.</h2>
          <p>Completa la informacion y la tarjeta aparecera inmediatamente en el catalogo.</p>
        </div>
        ...
        <div className="manager-fields">
          ...
          <div className="form-field form-field-wide">
            ...
            <textarea
              ...
              onChange={handleChange}
              required
              placeholder="Describe el producto, sus ingredientes y terminacion"
            />
          </div>
          <div className="form-field">
          ...
          <div className="form-field">
            ...
            <input
              ...
              onChange={handleChange}
              placeholder="Cumpleanos, aniversario"
            />
          </div>
        </div>
        ...
        <div className="image-field">
          ...
          <label className={imagePreview ? "image-dropzone has-image" : "image-dropzone"} htmlFor="product-image">
            {imagePreview ? (
              ...
              <span><strong>Seleccionar imagen</strong><small>JPG, PNG o WEBP hasta 2MB</small></span>
            )}
          </label>
          <input
            key={fileInputKey}
...
function OrderForm({ selectedProduct, products }: { selectedProduct: string; products: Product[] }) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const message = [
      "Hola, Tortas Kelita. Quiero solicitar una cotizacion:",
      "",
      `Nombre: ${form.name}`,
      `Telefono: ${form.phone}`,
  ...
  return (
    <form className="order-form" onSubmit={handleSubmit}>
      ...
      <p className="form-note">Se abrira WhatsApp con los datos listos para enviar.</p>
    </form>
  );
}
      ...
      <section id="panel" className="section admin-overview">
        <div className="admin-title-row">
          <div>
            ...
            <p>Controla productos, existencias e imagenes desde un solo lugar.</p>
          </div>
          <span className="admin-session"><i /> Administracion activa</span>
        </div>
        <div className="admin-stats">
          ...
          <article><span>Categorias</span><strong>{activeCategories}</strong><small>categorias activas</small></article>
        </div>
      </section>
...
export function KelitaSite() {
  ...
  const deleteProduct = (product: Product) => {
    ...
    const confirmed = window.confirm(`Deseas eliminar "${product.name}" del catalogo?`);
    if (!confirmed) return;
    ...
    setQuoteProduct((current) => current === product.name ? "" : current);
  };
      ...
      {isAdmin ? (
        ...
        <>
          <main>
            <section id="inicio" className="hero">
              ...
              <div className="hero-content">
                <span className="eyebrow eyebrow-light">Reposteria artesanal &middot; Villa Alemana</span>
                ...
                <div className="hero-actions">
                  ...
                  <a className="button button-glass" href="#pedido">Pedir cotizacion</a>
                </div>
              </div>
              <div className="hero-note"><span>Pedidos personalizados</span><strong>Para cada ocasion</strong></div>
            </section>
            ...
            <section id="historia" className="section story-section">
              <div className="section-heading story-heading">
                ...
                <p>Preparamos cada pedido desde cero, respetando los tiempos del bizcocho, el relleno y la decoracion. La receta puede cambiar; el cuidado, no.</p>
              </div>
              <div className="story-grid">
                <img src="/images/sublogo.png" alt="Tortas Kelita preparando una receta artesanal" loading="lazy" />
                <div className="values-list">
                  <article><span>01</span><div><h3>Hecho a mano</h3><p>Produccion artesanal para cuidar textura, frescura y terminacion.</p></div></article>
            ...
            <section id="catalogo" className="section catalog-section">
              <div className="section-heading catalog-heading">
                ...
                <div className="filters" aria-label="Filtrar catalogo">
                  {categories.map((item) => (
              ...
              {visibleProducts.length > 0 ? (
                ...
                <div className="empty-catalog"><strong>No hay productos en esta categoria.</strong><span>Vuelve a revisar pronto.</span></div>
              )}
            </section>

            <section id="pedido" className="section order-section">
              <div className="order-intro">
                <span className="eyebrow">Tu proxima celebracion</span>
                <h2>Cuentanos que tienes en mente.</h2>
                <p>Completa los datos esenciales. Recibiras una cotizacion y coordinaremos juntos los detalles.</p>
                <div className="order-contact"><strong>Horario de atencion</strong><span>Lunes a viernes &middot; 09:00 a 19:00</span></div>
              </div>
          ...
          <footer className="site-footer">
            <div className="footer-brand"><img src="/images/logo.jpeg" alt="Logo de Tortas Kelita" /><div><strong>Tortas Kelita</strong><span>La tradicion de la abuela</span></div></div>
            <div className="footer-links"><a href="#catalogo">Catalogo</a><a href="#pedido">Pedidos</a><a href="https://instagram.com/tortaskelita" target="_blank" rel="noreferrer">Instagram</a></div>
            <p>&copy; 2026 Tortas Kelita. Sitio desarrollado por Alejandro Echeverria y Brallan Reyes.</p>
          </footer>
        </>
      )}
